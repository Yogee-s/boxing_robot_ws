...
class App:
    def __init__(self, root, node):
        ...
        self.root.geometry("1000x850") # Increased height for new controls
        self.sequence = [] 
    ...
    def setup_sequencer(self):
        ...
        # NEW: Trace Motion Button
        self.btn_trace = tk.Button(tool_frame, text="Trace Motion (OFF)", bg="#ddd", command=self.toggle_trace)
    ...
    def record_point(self):
        # Record all 4 motor positions + speeds (Manual Snapshot)
        point = {
    ...
    def toggle_trace(self):
        ...
        if self.is_tracing:
            self.btn_trace.config(text="Trace Motion (ON)", bg="#9f9")
            # Start background recording thread
            threading.Thread(target=self._trace_loop, daemon=True).start()
        else:
            self.btn_trace.config(text="Trace Motion (OFF)", bg="#ddd")

    def _trace_loop(self):
        # Records at 20Hz while button is active
        while self.is_tracing:
            self.record_point()
            time.sleep(0.05) # 20Hz Sampling Rate

    def update_listbox(self):
        # Only update periodically or on small lists to avoid freezing UI during tracing
        if len(self.sequence) % 10 == 0 or len(self.sequence) < 20:
            self.listbox.delete(0, tk.END)
            self.listbox.insert(tk.END, f"... {len(self.sequence)} points recorded ...")
            # Show last few
            start_idx = max(0, len(self.sequence) - 5)
    ...
    def delete_point(self):
        if self.listbox.curselection():
            # This logic is tricky with the abbreviated list, so we clear for now if trace used
            self.sequence.pop() # Remove last
            self.update_listbox()
    ...
    def load_sequence(self):
        ...
        if f:
            ...
            self.listbox.delete(0, tk.END)
            self.listbox.insert(tk.END, f"Loaded {len(self.sequence)} points.")
    ...
    def _play_logic(self):
        ...
        for i, step in enumerate(self.sequence):
            ...
            # Set Targets
            p = step['pos']
            current_p = self.node.target_pos # Use commanded pos for delta calculation
            
            self.node.set_target_arm(0, p[0], p[1], step['spd_l'])
            self.node.set_target_arm(1, p[2], p[3], step['spd_r'])
            
            # Calculate Wait Time based on distance and speed
            # Find max distance any motor has to move
            max_dist = 0.0
            max_speed = 1.0
            for j in range(4):
                d = abs(p[j] - current_p[j])
                if d > max_dist: 
                    max_dist = d
                    # Use the speed associated with that arm
                    max_speed = step['spd_l'] if j < 2 else step['spd_r']
            
            if max_speed < 0.1: max_speed = 1.0
            
            duration = max_dist / max_speed
            
            # If duration is tiny (Trace Mode), use fixed minimum time step
            if duration < 0.05: 
                time.sleep(0.05) # 20Hz Playback rate matches Recording rate
            else:
                # If duration is long (Manual Keyframe), sleep calculated time
                time.sleep(duration * 1.1) # 10% buffer
