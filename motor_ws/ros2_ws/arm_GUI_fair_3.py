...
class RobotNode(Node):
    def __init__(self):
        ...
        # Publisher & Subscriber (Motor Control)
        ...
        # Action Status Publisher (Updated to Absolute Path)
        self.pub_action_status = self.create_publisher(String, '/robot/robot_action_status', 10)
        ...
        # Force topic advertisement
        self.get_logger().info("Node Initialized. Publishing 'GUI Ready' to /robot/robot_action_status...")
        msg = String()
        msg.data = "GUI Ready"
...
class ActionBoardTab:
    ...
    def setup_ui(self):
        ...
        for i in range(1, 7):
            ...
            btn.config(command=lambda idx=i: self.trigger_action(idx, is_remote=False))
    ...
    def trigger_action(self, idx, is_remote=False):
        ...
        if self.var_queue_mode.get():
            ...
            threading.Thread(target=self._play_single, args=(data, idx, is_remote), daemon=True).start()
        ...
        if msg is not None:
            ...
            if self.var_remote_enable.get():
                if msg in self.TRIGGER_MAPPING:
                    ...
                    self.trigger_action(slot, is_remote=True)
    ...
    def _play_single(self, data, idx, is_remote=False):
        self.is_playing = True
        try:
            success = self._execute_sequence_data(data)
            if success and is_remote:
                status_msg = String()
                status_msg.data = "0"
                self.node.pub_action_status.publish(status_msg)
        finally:
            self.is_playing = False
            ...
            while self.is_playing:
                if not self.action_queue:
                    ...
                    if self.var_loop.get() and self.lst_queue.size() > 0:
                        # Reload logic if looping implementation desired
                        break 
                    else:
                        break
                ...
                if success:
                    # Publish success signal for queue completion of this item
                    status_msg = String()
                    status_msg.data = "0"
                    self.node.pub_action_status.publish(status_msg)
...
def main():
    rclpy.init()
    node = RobotNode()
    print("Node Started. Topics: /motor_commands (Pub), /motor_feedback (Sub), /robot/robot_action_trigger (Sub), /robot/robot_action_status (Pub)")
