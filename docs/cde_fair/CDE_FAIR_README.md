...
## CDE AI Fair — Showcase Experience

This version was deployed as a **prototype** at the CDE AI Fair, where members of the public interacted with the robot in a live demonstration setting. The showcase revealed several real-world challenges that informed the design of the final system.

### Reaction Drill
- **Background clutter:** The drill was designed for a clean indoor setting with minimal background. The actual fair venue had many people walking behind the user, causing false detections.
  - **Fix:** Tuned the algorithm to track only the nearest person and ignore others in the background.
- **Variable punching stances:** Different users have different punching stances and speeds, making fixed velocity thresholds unreliable for reaction time measurement.
  - **Fix:** Changed the detection method to track hands moving closer to the camera (depth-based approach) rather than relying on velocity thresholds. This simplified the reaction time measurement and made it significantly more reliable across different users.

### Shadow Sparring (Color Tracking)
- **Color interference:** HSV color-based glove tracking was susceptible to background objects of similar color (clothing, signage, etc.) causing false positives.
  - **Mitigation:** Tuned HSV ranges and added spatial filtering to reduce false matches.
  - **Context:** Color tracking was used as the primary detection method because the main CV action recognition model was not ready in time for the fair. This limitation directly motivated the development of the Transformer-based action recognition model in the final version.

### Defence Drill
- **Actuation latency:** Noticeable delay between the Jetson sending commands and the arm completing movements. This was not a Jetson performance issue — the Teensy firmware intentionally limited motor speed to prevent damage to the arm mechanism during rapid movements.
  - **Impact:** Made the defence drill feel sluggish for users expecting fast robot attacks.

### LLM Coach
- **Cold start delay:** The Qwen 2.5 3B model took several seconds to load on first prompt, causing a noticeable wait for the first coaching response.
  - **Fix for final version:** Lazy-load the model when the GUI starts up rather than waiting for the first user prompt.
- **Input friction:** Typing questions on the touchscreen was awkward — users had to bend down to the 7" screen to type. Not practical in a standing boxing context.
  - **Fix for final version:** The phone dashboard allows users to type on their mobile device, which is far more natural and accessible.

### ROS Architecture
- **Camera sharing:** A key optimization challenge was sharing a single camera feed among multiple CV consumers (color tracking, reaction time pose estimation, main action model). Each consumer opening its own camera stream was wasteful and caused resource contention.
  - **Fix for final version:** Single camera node publishes to shared topics that all consumers subscribe to.

### Key Takeaways
These real-world deployment lessons directly shaped the final BoxBunny system:
1. Color tracking replaced with robust Transformer-based action recognition
2. Depth-based proximity detection for more reliable punch triggering
3. Background-robust person selection (nearest + largest person)
4. Phone dashboard for comfortable LLM interaction
5. Lazy model loading for instant coach availability
6. Full session lifecycle state machine instead of per-drill ad-hoc control
